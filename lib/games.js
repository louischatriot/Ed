...
function checkAndLaunch() {
  ...
  games[gameId] = new Level({ tileTableWidth: 10, tileTableHeight: 5 });
  games[gameId].createNewLevel();

  sockets.forEach(function (socket) {
    socket.emit('game.begun', { level: games[gameId].serialize(), startGameAfter: config.startGameAfter });
    socket.on('action', function () {
      // TODO: implement
      console.log("Received action from player socket " + socket.id);
    });
  });
}
