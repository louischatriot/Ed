...
function checkAndLaunch() {
  ...
  // STEP 2b: Called whenever a client tells the server it has prepared the game, launch game when all clients are prepared
  function checkEveryClientPrepared () {
    ...
    sockets.forEach(function (socket) {
      socket.on('action.jump', function (msg) {
        ...
        console.log(game.getPlayerById(socket.id).jumpStartedAt);   // TODO: use accurate jump data
  ...
  // STEP 2a: listen for prepared clients
  sockets.forEach(function (socket) {
  ...
  // STEP 1: send game data to clients
